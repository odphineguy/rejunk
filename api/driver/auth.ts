/**
 * Vercel serverless twin of the Express POST /api/driver/auth route — driver
 * activation, PIN login, session validation, and office-side create/revoke of
 * driver access. Requires SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY in the
 * Vercel env.
 *
 ...
 * kept in sync with server/driverAccess.ts (used by the Express route and the
 * Vite dev middleware). Security rationale lives in that file's header.
 */
...
type StaffCaller = { id: string; role: string; employeeId: string | null };

/** Resolves an office (staff) session token to an ACTIVE staff row, or null. */
async function resolveStaffToken(supabase: SupabaseClient, token: unknown): Promise<StaffCaller | null> {
...
async function handleDriverAction(body: unknown): Promise<Result> {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { status, body: out } = await handleDriverAction(body);
